// src/components/results/ModelModelComparisonDetailed.jsx
import React from "react";

const ModelModelComparisonDetailed = ({ items = [], category }) => {
              ...
              return (
                <tr key={idx} className="hover:bg-blue-50 transition-colors">
                    ...
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        item["Comparison Status"] === "Match"
                          ...
                          : item["Comparison Status"] === "Missing in Model A BOM"
                  ...
                  <td className="px-4 py-3 text-gray-700">
                    {details?.["DRG. NO. / DIMENSION"]?.["Model A BOM"] ||
                      details?.["DRG. NO. / DIMENSION"]?.["Model B BOM"] ||
                      "-"}
                  </td>
                  <td className="px-4 py-3">{details?.QTY?.["Model A BOM"] ?? "-"}</td>
                  <td className="px-4 py-3">{details?.QTY?.["Model B BOM"] ?? "-"}</td>
                  <td className="px-4 py-3">{details?.["REVISION NUMBER"]?.["Model A BOM"] ?? "-"}</td>
                  <td className="px-4 py-3">{details?.["REVISION NUMBER"]?.["Model B BOM"] ?? "-"}</td>
...
export default ModelModelComparisonDetailed;
