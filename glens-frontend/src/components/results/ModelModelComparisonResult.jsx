...
import ModelModelNewlyAdded from "../results/model_model_result/ModelModelNewlyAdded";
import ModelModelComparisonDetailed from "../results/model_model_result/ModelModelComparisonDetailed";
import ModelModelToCreateNew from "../results/model_model_result/ModelModelToCreateNew";

const ModelModelComparisonResult = ({ result }) => {
  ...
  const renderContent = (tab) => {
    if (tab === "UNMATCHED / MISSING" || tab ==="NEWLY ADDED")
      return <ModelModelNewlyAdded items={processedResults[tab]} category={tab} />;

    if (tab === "TO CREATE NEW" || tab === "EMPTY OR DASHED")
      return <ModelModelToCreateNew items={processedResults[tab]} category={tab} />;

    if (tab === "MATCHED") {
      const subItems = processedResults.MATCHED[activeSubTab] || [];
      return (
        <ModelModelComparisonDetailed
          items={subItems}
    ...
    <div className="h-[60vh] flex flex-col bg-white rounded-lg border shadow-sm">
      ...
      <div className="p-5 border-b bg-gray-50 rounded-t-lg">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
          <div><b>Model A File:</b> {result.model_a_file}</div>
          <div><b>Model B File:</b> {result.model_b_file}</div>
...
export default ModelModelComparisonResult;
