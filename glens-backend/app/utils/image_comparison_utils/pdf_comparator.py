...
from ._pdf_handler import PDFHandler
from ._utils_for_compare import CompareUtils, CharInfo
from ._utils_for_image import ImageUtils
...
def merge_image(
    ...
    # Determine minimum dimensions to align images
    h = min(a_image.shape[0], b_image.shape[0])
    w = min(a_image.shape[1], b_image.shape[1])

    # Resize the text-diff images to the same size
    a_text_image = cv2.resize(a_text_image, (w, h))
    b_text_image = cv2.resize(b_text_image, (w, h))
    upper = np.hstack((a_text_image, b_text_image))

    # Optional safety resize to stay within OpenCV's max image size (65500px)
    max_dim = 65000
    height, width = upper.shape[:2]
